...
export default class CreateBlendingRay {
  constructor(dom) {
    this.dom = dom;
  ...
  drawBlendingRay() {
    ...
    Loader.add({
      name: 'spritesheet',
      url: './img/optical/spritesheet.json',
    }).load((loader, resources) => {
      const imgs = resources.spritesheet.textures;
      const rayPen = new PIXI.Sprite(imgs['red-ray-pen.png']);
      rayPen.position.set(this.app.screen.width / 2 - 200, this.app.screen.height / 2 - 100);
      rayPen.rotation = 1;

      this.app.stage.addChild(rayPen);
    });
  }
